...
<br>
<small style="padding: 1px 9px 2px;font-weight: bold;white-space: nowrap;color: #ffffff;-webkit-border-radius: 9px;-moz-border-radius: 9px;border-radius: 9px;background-color: #3a87ad;">
...
```bash
curl -X {{$route['methods'][0]}} {{$route['methods'][0] == 'GET' ? '-G ' : ''}}"{{ trim(config('app.docs_url') ?: config('app.url'), '/')}}/{{ ltrim($route['uri'], '/') }}@if(!empty($route['cleanQueryParameters']))?{!! str_replace(array('%7B%7B', '%7D%7D'),array('{{', '}}'),http_build_query($route['cleanQueryParameters'])) !!}@endif" @if(count($route['headers']))\
